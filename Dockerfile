...
# install R packages
RUN Rscript -e 'p <- c("tidyverse", "tidyjson", "RColorBrewer", "tikzDevice");install.packages(p, repos="http://cran.rstudio.com/", clean=TRUE); for(x in p) { if (!require(x,character.only = TRUE)) {quit(1)}}'
